...
{
  int cow_enabled = p->cow_enabled;
  int cow_group = p->cow_group;
  if(p->trapframe)
...
growproc(int n)
{
  ...
  if (p->ondemand == true)
  {
    int no_of_pages = n / PGSIZE;
    print_skip_heap_region(p->name, p->sz, no_of_pages);
    track_heap(p, p->sz, no_of_pages);
    p->sz = p->sz + n;
    return 0;
  }
