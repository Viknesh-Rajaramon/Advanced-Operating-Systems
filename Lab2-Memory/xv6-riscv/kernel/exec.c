...
exec(char *path, char **argv)
{
  ...
  char sh[] = "sh\0";
  // COW tests
  char test8[] = "test8-cow1\0";
  char test9[] = "test9-cow2\0";
  char test10[] = "test10-cow3\0";

  // Statically allocate init and sh processes
  if (strncmp(path, init, strlen(init)) == 0 || strncmp(path, sh, strlen(sh)) == 0 || strncmp(path, test8, strlen(test8)) == 0 || strncmp(path, test9, strlen(test9)) == 0 || strncmp(path, test10, strlen(test10)) == 0)
    p->ondemand = false;
  else
    p->ondemand = true;
